...
The launcher resolves its own symlink with \code{readlink -f}. That lets it
find the real install directory, where \code{.env} and
\code{opencode-go-claude-proxy.mjs} live. Claude Code itself starts back in the
directory where you ran \code{claude-o-go}, so the current project remains the
working directory.
...
\section{Theory Excursion: Localhost, Ports, and Processes}

Networking starts with a simple address: where should a client connect?

\begin{itemize}
  \item \code{127.0.0.1} means localhost. It points back to the same machine.
  \item A port selects one listening process on that machine. The default proxy port is \code{4141}.
  \item A server binds to an address and port. The proxy binds to \code{127.0.0.1:4141}.
  \item A client connects to that address and port. Claude Code connects to the local proxy URL:
\end{itemize}
...
The proxy listens on localhost, not on \code{0.0.0.0}. That matters:
\code{127.0.0.1} is reachable only from your own machine, while
\code{0.0.0.0} would expose the listener on all network interfaces.
...
\section{Theory Excursion: HTTP Proxy vs Backend}

Claude Code is the client. OpenCode Go is the upstream backend. The local Node
script is a proxy.

A proxy receives a request from one side and sends a related request to another
side. It can preserve most of the request, rewrite selected fields, and relay
the response.

In this project the proxy does three important things:

\begin{enumerate}
  \item It accepts Claude Code's Anthropic-style request at \code{/v1/messages}.
  \item It rewrites \code{body.model} from Claude's local alias to the OpenCode Go model.
  \item It forwards the request to OpenCode Go and streams the response back.
\end{enumerate}

The proxy is not a model host. It does not generate tokens. It is glue code
between two APIs that are almost, but not perfectly, compatible.
...
\section{Theory Excursion: Streaming, SSE, and Backpressure}

LLM responses are usually streamed. The backend does not wait until the whole
answer is complete; it sends small chunks as they become available.

The common web pattern for this is SSE: Server-Sent Events. An SSE stream is
still HTTP, but the response body stays open and sends events over time:
...
Backpressure means the receiver cannot always consume data as fast as the
sender writes it. In Node, \code{response.write(chunk)} returns \code{false}
when the outgoing buffer is full. Correct proxy code waits for \code{drain}
before continuing. That prevents memory growth and unstable long-running
streams.

In this project:

\begin{itemize}
  \item Claude Code reads from the local proxy.
  \item The proxy reads from OpenCode Go.
  \item The proxy writes chunks to Claude Code while respecting Node stream backpressure.
  \item If Claude Code closes the local connection, the proxy aborts the upstream request.
\end{itemize}

Stream handling is more delicate than a simple JSON request. A normal request
has one response body. A stream is an ongoing conversation between sockets.

\section{Theory Excursion: Common Network Errors}

These error names are useful admin vocabulary:

\begin{itemize}
  \item \code{ECONNREFUSED}: nothing accepted the connection. Typical causes: wrong port, proxy not running, or upstream unreachable.
  \item \code{ECONNRESET}: the other side reset an existing connection. Typical cause: the upstream closed the socket mid-stream.
  \item \code{UND\_ERR\_SOCKET}: Node's Undici HTTP client saw a socket failure. Usually the same class as a reset or broken stream.
  \item \code{ETIMEDOUT}: connection or response took too long. Typical cause: network path or backend too slow.
  \item \code{AbortError}: we intentionally aborted the request. Typical cause: Claude Code closed the local connection.
\end{itemize}

The practical rule: retry before response headers are sent; after streaming has
started, finish the stream cleanly with an error event. Once headers and
partial content have been sent, the proxy cannot turn the response into a
normal JSON \code{502}.
...
Claude Code. It handles Node stream backpressure, aborts the upstream request
if Claude Code closes the local connection, retries retryable upstream
connection failures before response headers are sent, and turns mid-stream
upstream resets into a clean streaming error event instead of crashing the
proxy process.
...
Proxy request logs are hidden during normal use so Claude Code's TUI stays
clean. With \code{OPENCODE\_GO\_PROXY\_LOG=1}, look for lines like:
...
\subsection{Proxy Logs Show Up in Claude Code's TUI}
